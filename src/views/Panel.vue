...
onMounted(async ()=>{ 
  /*
    let respuesta =  await fetch(`${import.meta.env.VITE_API_URL}recetas-helpers/usuario/${localStorage.getItem('recetas_flaites_id')}`, {
        headers:{'Content-Type':'application/json', 'Authorizacion':`Bearer ${localStorage.getItem('recetas_flaites_token')}` }
    }); 
    datos.value = await respuesta.json();
    */
    datos.value = await axios.get(`${import.meta.env.VITE_API_URL}recetas-helpers/usuario/${localStorage.getItem('recetas_flaites_id')}`, {headers:{'Content-Type':'application/json', 'Authorization': `Bearer ${localStorage.getItem('recetas_flaites_token')}`}})
        .then((response)=>{
           datos = response.data;
        });
                  ...
                  <tbody>
                    <tr v-for="(dato, index) in datos" :key="index">
                        ...
                        <td class="text-center">
                            <a :href="dato.foto" class="lightbox d-block" data-fancybox="image-gallery">
                                <img :src="dato.foto" :alt="dato.nombre" style="width: 100px;" />
                            </a>
                        </td>
                        <td class="text-center">
